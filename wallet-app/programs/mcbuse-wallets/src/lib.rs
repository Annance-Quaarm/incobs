...
declare_id!("7V3amFUk7nFV2rt8ucrupRA3nGGEmGTu4CXqiVfFqt9U");
...
pub mod mcbuse_wallets {
    ...
    pub fn initialize_reserve_wallet(context: Context<InitializeReserveWallet>, wallet_name: String, threshold: u64, bump: u8) -> Result<()> {
        ...
        msg!("Reserve wallet created with threshold: {} SOL", threshold as f64 / 1_000_000_000.0);
    ...
    pub fn deposit(context: Context<Deposit>, amount: u64) -> Result<()> {
        ...
        if let Some(contribution) = reserve_wallet.user_contributions.iter_mut()
        ...
        if reserve_wallet.wallet_balance >= reserve_wallet.threshold && !reserve_wallet.bank_account_created {
        ...
        if let Some(bank_account_address) = reserve_wallet.bank_account_address {
            ...
            msg!("Deposit of {} SOL received from user {}. No bank account created yet.", 
                 amount as f64 / 1_000_000_000.0,
                 depositor);
        }
    

        msg!("Deposit of {} SOL received from user {}. New balance: {} SOL",
              amount as f64 / 1_000_000_000.0,
              depositor,
    ...
    pub fn approve_bank_account_creation(context: Context<ApproveBankAccountCreation>) -> Result<()> {
    ...
    pub fn set_bank_account_created(context: Context<SetBankAccountCreated>, bank_account_address: Pubkey) -> Result<()> {
        ...
        **context.accounts.bank_wallet.to_account_info().try_borrow_mut_lamports()? += transfer_amount;
        
        reserve_wallet.bank_account_created = true;
        reserve_wallet.bank_account_address = Some(bank_account_address);
        
        // Log event for API
        msg!("BANK_ACCOUNT_CREATED:{}:{}:{}",
            reserve_wallet.name,
            bank_account_address,
            transfer_amount
        );
        ...
        msg!("Bank account created for reserve wallet: {}", reserve_wallet.name);
    ...
    pub fn withdraw_funds(context: Context<WithdrawFunds>, amount: u64) -> Result<()> {
        ...
        let user_contribution_option = reserve_wallet.user_contributions.iter()
        ...
        let seeds = &[b"reserve_wallet".as_ref(), reserve_wallet.name.as_bytes(), &[reserve_wallet.bump]];
        ...
        if let Some(contribution) = reserve_wallet.user_contributions.iter_mut()
                                    .find(|contribution| contribution.user == withdrawer) {
                                        ...
                                        if contribution.amount == 0 {
                                            if let Some(pos) = reserve_wallet.user_contributions.iter().position(|contribution| contribution.user == withdrawer) {
    ...
    pub fn transfer_from_bank_to_wallet(context: Context<TransferFromBankToWallet>, amount: u64) -> Result<()> {
    ...
    pub fn sync_wallet_balance(context: Context<SyncWalletBalance>) -> Result<()> {
        ...
        let current_balance = **context.accounts.reserve_wallet.to_account_info().lamports.borrow();
        
        let reserve_wallet = &mut context.accounts.reserve_wallet;
        reserve_wallet.wallet_balance = current_balance;
        
        msg!("Wallet balance synced: {} SOL", 
...
pub struct InitializeReserveWallet<'info> {
    ...
    #[account(mut)] // mutable because the system program will deduct lamports from OUR account(we are sponsoring? feasible? our business to B2B2C)
